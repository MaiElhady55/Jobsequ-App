...
import 'package:jobsque/features/profile/data/models/NotificationSettingsModel.dart';
import 'package:jobsque/features/profile/data/models/portfolio_model.dart';
import 'package:jobsque/features/profile/data/models/profile_model.dart';
...
class ProfileCubit extends Cubit<ProfileState> {
  ...
  void addItem(String item) {
    ...
    if (completeProfile.length == 4) {
      CachHelper.saveDate(
          key: MySharedKeys.completeProfile.toString(), value: 'true');
    }
  ...
  getProfileDetailsAndPortfolios() {
    ...
    DioHelper.getData(endPoint: portofolioUrl, token: token).then((value) {
      // print(value.data);
      for (var portfolio in value.data['data']['portfolio']) {
        portfolios.add(Portfolio.fromJson(portfolio));
        print(portfolio);
      }
      // for (var profile in value.data['data']['profile']) {
      //   profileDetails.add(Profile.fromJson(profile));
      //   print('profileDetails ${profileDetails[0]}');
      // }
      profileDetails.add(Profile.fromJson(value.data['data']['profile']));
      print('profileDetails ${profileDetails[0].email}');
